...
const MaterialSalesReport = () => {
  ...
  const [materialStockInfo, setMaterialStockInfo] = useState<
    MaterialStockInfo[]
  >([]);
  ...
  const [viewMode, setViewMode] = useState<"summary" | "details" | "stock">(
    "summary"
  );
  ...
  const fetchMaterialStockInfo = async () => {
    try {
      ...
      const { data: stockData, error: stockError } = await supabase.from(
        "materials"
      ).select(`
                    id,
                    name,
      ...
      const stockInfo: MaterialStockInfo[] = (stockData || []).map(
        (material) => {
          const imports = material.material_imports || [];
          const sales = (salesData || []).filter(
            (sale) => sale.material_id === material.id
          );

          const totalImported = imports.reduce(
            (sum, imp) => sum + parseFloat(imp.quantity || 0),
            0
          );
          const totalSold = sales.reduce(
            (sum, sale) => sum + parseFloat(sale.quantity || 0),
            0
          );
          const totalImportCost = imports.reduce(
            (sum, imp) => sum + parseFloat(imp.total_amount || 0),
            0
          );
          const avgImportPrice =
            totalImported > 0 ? totalImportCost / totalImported : 0;
          ...
          return {
            ...
            stock_value:
              parseFloat(material.current_stock || 0) * avgImportPrice,
          };
        }
      );
    ...
    try {
      ...
      const filteredData = detailsData.filter((item: any) => {
        ...
        const isSelectedMaterial =
          selectedMaterial === "all" || item.material_id === selectedMaterial;
      ...
      const formattedDetails: MaterialSaleDetail[] = filteredData.map(
        (item: any) => ({
          id: item.id,
  ...
  const calculateMaterialSummaries = (
    details: MaterialSaleDetail[]
  ): MaterialSummary[] => {
    ...
    summaryMap.forEach((summary, key) => {
      ...
      summary.total_orders = new Set(
        materialDetails.map((d) => d.order_id)
      ).size;
      summary.unique_customers = new Set(
        materialDetails.map((d) => d.customer_name)
      ).size;
      ...
      summary.avg_unit_price =
        summary.total_quantity_sold > 0
          ...
          : 0;
      summary.avg_cost_price =
        summary.total_quantity_sold > 0
          ? summary.total_cost / summary.total_quantity_sold
          : 0;
    });
  ...
  const filteredDetails = materialDetails.filter(
    (detail) =>
  ...
  const filteredSummaries = materialSummaries.filter((summary) => {
    ...
    const combinedText = `${summary.material_name}`.toLowerCase();
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  Phân tích bán hàng và tồn kho vật tư
                </p>
              </div>
            </div>
          ...
          {[
            {
              ...
              value: formatCurrency(
                materialSummaries.reduce((sum, m) => sum + m.total_revenue, 0)
              ),
              icon: DollarSign,
            ...
            },
            {
              ...
              value: formatCurrency(
                materialSummaries.reduce((sum, m) => sum + m.total_profit, 0)
              ),
              icon: TrendingUp,
            ...
            },
            {
              ...
              value: formatCurrency(
                materialStockInfo.reduce((sum, m) => sum + m.stock_value, 0)
              ),
              icon: Package,
