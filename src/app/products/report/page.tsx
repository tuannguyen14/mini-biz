...
import { Calendar, TrendingUp, Package, Users, DollarSign, Eye, Download, Filter, Search, RefreshCw } from 'lucide-react';
...
const ProductSalesReport = () => {
    ...
    const fetchProductSalesData = async () => {
        ...
        `)
            ...
            const filteredData = detailsData.filter((item: any) => {
                ...
                const isSelectedProduct = selectedProduct === 'all' || item.product_id === selectedProduct;

                return isInDateRange && isSelectedProduct && item.products && item.orders?.customers;
            });

            const formattedDetails: ProductSaleDetail[] = filteredData.map((item: any) => ({
                id: item.id,
    ...
    const calculateProductSummaries = (details: ProductSaleDetail[]): ProductSummary[] => {
        ...
        summaryMap.forEach((summary, key) => {
            ...
            summary.total_orders = new Set(productDetails.map(d => d.order_id)).size;
            summary.unique_customers = new Set(productDetails.map(d => d.customer_name)).size;
    ...
    const exportToCSV = () => {
        ...
        const headers = viewMode === 'summary'
            ? ['Sản phẩm', 'Số lượng bán', 'Doanh thu', 'Chi phí', 'Lợi nhuận', 'Tỷ suất LN (%)', 'Số đơn hàng', 'Số khách hàng']
            : ['Sản phẩm', 'Ngày bán', 'Khách hàng', 'SĐT', 'Số lượng', 'Đơn giá', 'Thành tiền', 'Chi phí', 'Lợi nhuận'];
        ...
        const csvContent = [
            ...
            ...data.map(row => {
                if (viewMode === 'summary') {
                    ...
                    return [s.product_name, s.total_quantity_sold, s.total_revenue, s.total_cost, s.total_profit, s.profit_margin.toFixed(2), s.total_orders, s.unique_customers].join(',');
                } else {
                    const d = row as ProductSaleDetail;
                    return [d.product_name, formatDate(d.order_date), d.customer_name, d.customer_phone, d.quantity, d.unit_price, d.total_price, d.total_cost, d.profit].join(',');
                }
            })
        ].join('\n');
                ...
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 mb-8">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <h1 className="text-3xl font-bold text-gray-800">Báo cáo chi tiết sản phẩm</h1>
                                <p className="text-gray-600 mt-1">Phân tích doanh thu và lợi nhuận theo sản phẩm</p>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                        ...
                        {/* Product Filter */}
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-3">Sản phẩm</label>
                            <select
                        ...
                        <div>
                            ...
                            <div className="flex rounded-lg border border-gray-300 overflow-hidden">
                                <button
                                    ...
                                    className={`flex-1 px-4 py-2 text-sm font-medium transition-colors ${viewMode === 'summary'
                                ...
                                <button
                                    ...
                                    className={`flex-1 px-4 py-2 text-sm font-medium transition-colors ${viewMode === 'details'
                                            ? 'bg-blue-500 text-white'
                                            : 'bg-white text-gray-700 hover:bg-gray-50'
                                        }`}
                                >
                    ...
                    {[
                        {
                            ...
                            value: formatCurrency(productSummaries.reduce((sum, p) => sum + p.total_revenue, 0)),
                            icon: DollarSign,
                            color: 'from-green-500 to-emerald-600'
                        },
                        {
                            label: 'Tổng lợi nhuận',
                            value: formatCurrency(productSummaries.reduce((sum, p) => sum + p.total_profit, 0)),
                            icon: TrendingUp,
                        ...
                        {
                            label: 'Đơn hàng',
                            value: new Set(productDetails.map(d => d.order_id)).size.toString(),
                            icon: Users,
                ...
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-200">
                        <h2 className="text-xl font-bold text-gray-800">
                            {viewMode === 'summary' ? 'Tổng quan theo sản phẩm' : 'Chi tiết bán hàng'}
                        </h2>
                    </div>
                                ...
                                <table className="w-full">
                                    <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
                                        <tr>
                                            ...
                                            <th className="px-6 py-4 text-right text-sm font-semibold text-gray-700">Khách hàng</th>
                                        </tr>
                                    </thead>
                                        ...
                                        {filteredSummaries.map((summary, index) => (
                                            <tr key={index} className="hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 transition-all duration-200">
                                                <td className="px-6 py-4">
                                                    <div className="font-medium text-gray-900">{summary.product_name}</div>
                                                </td>
                                ...
                                <table className="w-full">
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {filteredDetails.map((detail, index) => (
                                            <tr key={index} className="hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 transition-all duration-200">
                                                <td className="px-6 py-4">
                                                    <div className="font-medium text-gray-900">{detail.product_name}</div>
                                                    <div className="text-sm text-gray-500">{detail.product_unit}</div>
                                                </td>
                    ...
                    {/* Empty State */}
                    {!loading && (viewMode === 'summary' ? filteredSummaries.length === 0 : filteredDetails.length === 0) && (
                        <div className="text-center py-12">
                            <Package className="h-16 w-16 text-gray-300 mx-auto mb-4" />
                            <h3 className="text-lg font-medium text-gray-900 mb-2">Không có dữ liệu</h3>
                            <p className="text-gray-500">Không tìm thấy dữ liệu bán hàng trong khoảng thời gian đã chọn.</p>
                        </div>
                    )}
                </div>
            </div>
